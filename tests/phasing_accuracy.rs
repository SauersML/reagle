...
#[test]
fn test_dynamic_mcmc_heuristic() {
    let n_markers = 20;
    let n_ref_haps = 40;
    let hero_idx = 10;
    ...
    // Verify low switch error rate (should find the all-0 / all-1 pair)
    let hero_pattern = hero_pattern_from_ref_hap(&ref_haps, hero_hap_idx);
    let mut switch_errors = 0;
    for m in 1..n_markers {
        let hero_prev = hero_pattern[m - 1];
    ...
    let ser = switch_errors as f32 / (n_markers - 1) as f32;
    println!("Dynamic MCMC Heuristic SER: {:.4}", ser);

    // It should perform well
    assert!(ser < 0.1, "Dynamic MCMC SER too high: {:.4}", ser);
}
