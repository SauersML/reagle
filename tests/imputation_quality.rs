...
fn parse_gp_max(field: Option<Value>) -> Option<f64> {
    match field {
        Some(Value::Array(Array::Float(vals))) => vals
            .iter()
...
fn run_metrics(truth_path: &Path, imputed_path: &Path, output_json: &Path) {
    let mut truth_reader =
        vcf::io::Reader::new(BufReader::new(File::open(truth_path).expect("Open Truth")));
    let truth_header = truth_reader.read_header().expect("Read Truth Header");

    let mut imp_reader = vcf::io::Reader::new(BufReader::new(
        File::open(imputed_path).expect("Open Imputed"),
    ));
    ...
    while let (Some(Ok(t)), Some(Ok(i))) = (truth_iter.peek(), imp_iter.peek()) {
        ...
        if let (Some(tgt), Some(igt)) = (t_gt, i_gt) {
            let truth_dose = parse_genotype_dose(
                tgt.get(&truth_header, "GT")
                    .transpose()
                    .ok()
                    .flatten()
                    .flatten(),
            ...
            let imp_dose = parse_dosage(
                igt.get(&imp_header, "DS")
                    .transpose()
                    .ok()
                    .flatten()
                    .flatten(),
    ...
    for d in &metrics_data {
        ...
        cal_bins[bin_idx].push(if (d.ds.round() - d.gt_dose).abs() < 0.1 {
            1.0
        } else {
            0.0
        });
