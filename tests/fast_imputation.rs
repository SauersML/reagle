...
fn test_synthetic_slam_dunk() {
    ...
    // So truth at target odd markers is 0.0 (S0) and 2.0 (S1).
...
fn test_ultra_dense_markers() {
    ...
    // Target with every 10th marker genotyped - Different haplotypes for different samples
    let target_file = SyntheticVcfBuilder::new(n_ref_markers, 2)
        .positions(positions)
        .allele_generator(|m, h| {
            if m % 10 == 0 {
                // Sample 0 (haps 0,1) matches group 0 (allele 0)
                // Sample 1 (haps 2,3) matches group 1 (allele 1)
                if h < 2 { 0 } else { 1 }
            } else {
                255
            }
    ...
    // Sample 0 (group 0) should be ~0.0
    // Sample 1 (group 1) should be ~2.0
    let mut sum_diff = 0.0;
    let mut count = 0;
    for marker_dosages in &dosages {
        let d0 = marker_dosages[0];
        let d1 = marker_dosages[1];
        sum_diff += d0.abs() + (d1 - 2.0).abs();
        count += 1;
    }

    let avg_diff = sum_diff / count as f32;
    println!("Average dosage diff: {}", avg_diff);
    assert!(
        avg_diff < 0.1,
        "Dosages should match haplotype groups, avg diff: {}",
        avg_diff
    );
