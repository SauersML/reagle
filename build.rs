...
fn manually_check_for_unused_variables() {
    ...
    if !build_path.exists() {
        emit_stage_detail("manual lint self-check: build script source not found (skipping check)");
        if warnings_enabled() {
            println!(
    ...
    let deps_dir = match build_dependencies_directory() {
        ...
        None => {
            emit_stage_detail(
                "manual lint self-check: could not determine build dependency directory (skipping check)",
            );
            if warnings_enabled() {
                println!(
                    "cargo:warning=manual lint self-check skipped: unable to derive build dependency directory from OUT_DIR"
                );
            }
            return;
        }
    ...
    let source_path = match manual_lint_args.pop() {
        ...
        None => {
            emit_stage_detail(
                "manual lint self-check: unable to obtain source path from manual lint arguments (skipping check)",
            );
            return;
        }
    };
        ...
        match locate_build_dependency(&deps_dir, crate_name) {
            ...
            None => {
                emit_stage_detail(&format!(
                    "manual lint self-check: missing rlib for dependency '{crate_name}' (skipping check)"
                ));
                if warnings_enabled() {
                    println!(
    ...
    match status {
        Ok(output) => {
            ...
            if !output.status.success() {
                ...
                if stderr.contains("unused variable") {
                    ...
                    emit_stage_detail(&format!(
                        "manual lint self-check warning: rustc self-lint exited with non-lint error status {}",
                        output
                            .status
                            .code()
                    ...
                    if warnings_enabled() {
                        println!(
                            "cargo:warning=manual lint self-check failed (check build logs for details)"
                        );
                    }
                    return;
                }
        ...
        Err(err) => {
            ...
            if warnings_enabled() {
                println!(
                    "cargo:warning=manual lint self-check skipped: failed to spawn rustc: {err}"
                );
            }
            return;
        }
