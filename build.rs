...
fn manually_check_for_unused_variables() {
    ...
    if !build_path.exists() {
        emit_stage_detail("manual lint self-check: build script source not found; skipping self-check");
        if warnings_enabled() {
            println!("cargo:warning=manual lint self-check skipped: build script source file {:?} is missing", build_path);
        }
        return;
    }
    ...
    let deps_dir = match build_dependencies_directory() {
        ...
        None => {
            emit_stage_detail(
                "manual lint self-check: could not determine build dependency directory; skipping self-check",
            );
            if warnings_enabled() {
    ...
    let source_path = match manual_lint_args.pop() {
        ...
        None => {
            emit_stage_detail(
                "manual lint self-check: unable to obtain source path from manual lint arguments; skipping self-check",
            );
            if warnings_enabled() {
                println!("cargo:warning=manual lint self-check skipped: internal argument assembly error");
            }
            return;
        }
    };
        ...
        match locate_build_dependency(&deps_dir, crate_name) {
            ...
            None => {
                emit_stage_detail(&format!(
                    "manual lint self-check: missing rlib for dependency '{crate_name}'; skipping self-check"
                ));
                if warnings_enabled() {
                    println!("cargo:warning=manual lint self-check skipped: required dependency '{crate_name}' rlib not found in {:?}", deps_dir);
                }
                return;
            }
        }
    ...
    match status {
        Ok(output) => {
            ...
            if !output.status.success() {
                ...
                if stderr.contains("unused variable") {
                    ...
                    emit_stage_detail(&format!(
                        "manual lint self-check warning: rustc self-lint exited with status {}. This might be due to environment differences.",
                        output.status.code().map(|c| c.to_string()).unwrap_or_default()
                    ));
                    if warnings_enabled() {
                         println!("cargo:warning=manual lint self-check failed to run correctly (exit status {:?}). Skipping.", output.status.code());
                         if !output.stderr.is_empty() {
                            println!("cargo:warning=rustc stderr: {}", stderr.replace('\n', " "));
                         }
                    }
                    // Do not exit(1) here, as this might be a spurious environment issue
                }
            } else {
        ...
        Err(err) => {
            ...
            if warnings_enabled() {
                println!("cargo:warning=manual lint self-check failed to spawn rustc: {}", err);
            }
            // Do not exit(1)
        }
    }
