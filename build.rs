...
fn manually_check_for_unused_variables() {
    ...
    let source_path = match manual_lint_args.pop() {
        ...
        None => {
            ...
            println!(
                "cargo:warning=manual lint self-check skipped: manual lint argument assembly failed to include the source path"
            );
            return;
        }
    };
    ...
    match status {
            ...
            if !output.status.success() {
                ...
                if stderr.contains("unused variable") {
                    ...
                    if !output.stderr.is_empty() {
                        println!("cargo:warning=rustc self-lint stderr: {}", stderr.replace('\n', " || "));
                    }
                    return;
                }
            } else {
        ...
        Err(err) => {
            ...
            println!(
                "cargo:warning=manual lint self-check skipped: failed to spawn rustc self-lint command: {err}"
            );
            return;
        }
    }
