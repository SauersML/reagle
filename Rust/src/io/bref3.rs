...
impl WindowedBref3Reader {
    ...
    /// 
    /// Accepts multiple chromosome candidates (aliases) to match against the file content.
    pub fn load_window_for_region(&mut self, chroms: &[String], start_pos: u32, end_pos: u32) -> Result<Option<RefWindow>> {
        ...
        let chrom = if let Some(current) = &self.current_chrom {
            if !chroms.iter().any(|c| c == current.as_ref()) {
                // Changing chromosome
                self.block_buffer.clear();
                self.current_chrom = None;
                // Will detect new chrom below
            }
            // Use current if it matches one of the candidates (we don't know which one yet if None, but here we do)
        } else {
            // New chromosome logic handled below
        };
        let _ = chrom; // unused
        ...
        while !self.inner.is_eof() {
            ...
            // Check if block matches any candidate
            let is_match = chroms.iter().any(|c| c == &next_block.chrom);
            
            if !is_match {
                // If we have a current chrom and this block doesn't match, it's the next chrom.
                // Stop loading.
                if !self.block_buffer.is_empty() {
                    self.pending_block = Some(next_block);
                    break;
                }
                // If we don't have a buffer yet, this block is a mismatch (e.g. "1" vs "22").
...
impl InMemoryRefReader {
    ...
    pub fn load_window_for_region(&mut self, chroms: &[String], start_pos: u32, end_pos: u32) -> Result<Option<RefWindow>> {
        ...
        // Find matching chromosome index
    ...
    pub fn load_window_for_region(&mut self, chroms: &[String], start_pos: u32, end_pos: u32) -> Result<Option<RefWindow>> {
        ...
        loop {
            while !self.eof {
                ...
                let is_match = chroms.iter().any(|c| c == marker_chrom);
            ...
            if after < BUFFER_MARKERS && !self.eof {
                ...
                let marker_chrom = self.markers.chrom_name(next_marker.marker.chrom).unwrap_or("");
                if !chroms.iter().any(|c| c == marker_chrom) {
                    self.pending_marker = Some(next_marker);
                    break;
                }
                self.buffer.push_back(next_marker);
