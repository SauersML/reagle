...
impl StreamingBref3Reader {
    ...
    pub fn prescan_markers(path: &Path) -> Result<Markers> {
        ...
        while let Some((chrom_name, block_markers)) = reader.next_block_prescan()? {
    ...
    fn next_block_prescan(&mut self) -> Result<Option<(String, Vec<Marker>)>> {
        if self.eof {
            return Ok(None);
        }
...
impl StreamingRefVcfReader {
    ...
    /// Pre-scan a VCF file to get all marker metadata without loading genotypes.
    pub fn prescan_markers(path: &Path) -> Result<Markers> {
        let mut reader = Self::open(path)?;
        let mut all_markers = Markers::new();

        while let Some(marker) = reader.read_next_marker_only()? {
            // Re-map chromosome index to the new Markers object
    ...
    /// Read the next marker's metadata only, skipping genotype data.
    fn read_next_marker_only(&mut self) -> Result<Option<Marker>> {
        loop {
            self.line_buf.clear();
            if self.reader.read_line(&mut self.line_buf)? == 0 {
                self.eof = true;
            ...
            // Split only up to ALT field - more efficient
            ...
            if alt_str.is_empty() { continue; } // Malformed line
