...
impl WindowedBref3Reader {
    ...
    ///
    /// Accepts multiple chromosome aliases (e.g. "22", "chr22") to handle naming differences.
    pub fn load_window_for_region(&mut self, chroms: &[String], start_pos: u32, end_pos: u32) -> Result<Option<RefWindow>> {
        ...
        // Check if current chromosome matches any of the candidates
        let current_matches = self.current_chrom.as_ref()
            .map(|c| chroms.iter().any(|cand| cand == c.as_ref()))
            .unwrap_or(false);
        ...
        if let Some(pending) = self.pending_block.take() {
            if chroms.iter().any(|c| c == &pending.chrom) {
        ...
        while !self.inner.is_eof() {
            ...
            let matches = chroms.iter().any(|c| c == &next_block.chrom);
            if matches {
                if self.current_chrom.is_none() {
            ...
            // If we have a current chrom established, and this block doesn't match it:
            // - If it matches OTHER candidates, that's weird (interleaved chroms?), but we treat as mismatch.
            // - If it doesn't match any candidates, it's a mismatch.
            // Basically: if we are locked to a chrom, we stop when we see something else.
            // If we are NOT locked, we skip until we find a match.

            if let Some(curr) = &self.current_chrom {
                if next_block.chrom != curr.as_ref() {
                ...
                // Not locked yet
                if !matches {
                    // Skip irrelevant chromosome block
                    continue;
                }
                // matches == true, so we set current_chrom above.
            }
        ...
        if !self.inner.is_eof() {
            ...
            // Only append if it matches the CURRENT chromosome
            if let Some(curr) = &self.current_chrom {
                if next_block.chrom == curr.as_ref() {
                    self.block_buffer.push_back(next_block);
                } else {
                 ...
                 // Should not happen if we found any blocks, but if buffer is empty and we just found one...
                 if chroms.iter().any(|c| c == &next_block.chrom) {
                     self.current_chrom = Some(Arc::from(next_block.chrom.as_str()));
                     self.block_buffer.push_back(next_block);
                 } else {
                     self.pending_block = Some(next_block);
        ...
        let current_chrom_name = self.current_chrom.as_deref().unwrap_or("");

        for block in &self.block_buffer {
            if block.chrom != current_chrom_name {
                continue;
            }
...
impl RefPanelReader {
    ...
    pub fn load_window_for_region(&mut self, chroms: &[String], start_pos: u32, end_pos: u32) -> Result<Option<RefWindow>> {
        match self {
            RefPanelReader::Bref3(r) => r.load_window_for_region(chroms, start_pos, end_pos),
            RefPanelReader::InMemory(r) => r.load_window_for_region(chroms, start_pos, end_pos),
            RefPanelReader::StreamingVcf(r) => r.load_window_for_region(chroms, start_pos, end_pos),
        }
    }
}
...
impl InMemoryRefReader {
    ...
    pub fn load_window_for_region(&mut self, chroms: &[String], start_pos: u32, end_pos: u32) -> Result<Option<RefWindow>> {
        ...
        let target_chrom_idx = self
            ...
            .chrom_names()
            .iter()
            .position(|name| chroms.iter().any(|c| c == name.as_ref()))
...
impl StreamingRefVcfReader {
    ...
    pub fn load_window_for_region(&mut self, chroms: &[String], start_pos: u32, end_pos: u32) -> Result<Option<RefWindow>> {
        ...
        if let Some(pending) = self.pending_marker.take() {
            ...
            if chroms.iter().any(|c| c == pending_chrom) {
                if self.current_chrom.is_none() {
        ...
        loop {
            while !self.eof {
                ...
                let matches = chroms.iter().any(|c| c == marker_chrom);
            ...
            if after < BUFFER_MARKERS && !self.eof {
                ...
                if let Some(curr) = &self.current_chrom {
                    ...
                    // Should be locked by now if we have items in buffer
                    // But if buffer only has matching items...
                    // Check logic again.
                    // If buffer matches, current_chrom is set.
                    // So we must be in current_chrom.
                    if marker_chrom != self.current_chrom.as_ref().unwrap().as_ref() {
                         self.pending_marker = Some(next_marker);
                         break;
                    }
                }
