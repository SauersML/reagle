...
impl MarkerImputationStats {
    ...
    pub fn dr2(&self, allele: usize) -> f32 {
        ...
        if var_d <= 0.0 || var_expected <= 0.0 {
            ...
            (var_d / var_expected).clamp(0.0, 1.0)
        }
    }
...
impl ImputationQuality {
    ...
    /// Record imputation accuracy for a sample at a marker
    ///
    /// This is used to accumulate quality statistics for validation.
    /// For biallelic markers, this records whether the imputed alleles match the true alleles.
    pub fn record_imputed(&mut self, marker: usize, hap1_correct: bool, hap2_correct: bool) {
        if let Some(stats) = self.marker_stats.get_mut(marker) {
            // For biallelic markers, we can estimate accuracy from correctness
            // This is a simplified version - full implementation would use posterior probabilities
            let p1_correct = if hap1_correct { 1.0 } else { 0.0 };
            let p2_correct = if hap2_correct { 1.0 } else { 0.0 };

            // Add as biallelic sample with estimated posterior probabilities
            stats.add_sample_biallelic(p1_correct, p2_correct, None);
        }
    }
}
...
impl VcfWriter {
        ...
        B: Fn(usize, usize) -> (u8, u8),
    {
        ...
        /// Structured VCF format buffer to avoid string manipulation bugs
        struct VcfFormatBuffer<'a> {
            buf: &'a mut String,
            ryu_buf: &'a mut ryu::Buffer,
            field_count: usize,
        }

        impl<'a> VcfFormatBuffer<'a> {
            fn new(buf: &'a mut String, ryu_buf: &'a mut ryu::Buffer) -> Self {
                Self {
                    buf,
                    ryu_buf,
                    field_count: 0,
            ...
            /// Start a new sample's genotype field
            ...
            /// Add a genotype string (e.g., "0|1")
            fn add_gt(&mut self, a1: u8, a2: u8) {
                self.field_sep();
                self.buf.push((b'0' + a1) as char);
                self.buf.push('|');
                self.buf.push((b'0' + a2) as char);
            }

            /// Add a dosage value
            fn add_ds(&mut self, ds: f32) {
                self.field_sep();
                self.buf.push_str(Self::format_f32_4dp(ds, self.ryu_buf));
            }

            /// Add genotype probabilities (GP field)
            fn add_gp(&mut self, n_alleles: usize, p1: Option<&crate::pipelines::imputation::AllelePosteriors>, p2: Option<&crate::pipelines::imputation::AllelePosteriors>) {
                self.field_sep();

                if let (Some(p1), Some(p2)) = (p1, p2) {
                    // Add haplotype 1 probabilities
                    for a in 1..n_alleles {
                        if a > 1 { self.buf.push(','); }
                        self.buf.push_str(Self::format_f32_4dp(p1.prob(a), self.ryu_buf));
                    }
                    if n_alleles <= 1 { self.buf.push_str("0.00"); }

                    self.buf.push(':');

                    // Add haplotype 2 probabilities
                    for a in 1..n_alleles {
                        if a > 1 { self.buf.push(','); }
                        self.buf.push_str(Self::format_f32_4dp(p2.prob(a), self.ryu_buf));
                    }
                    if n_alleles <= 1 { self.buf.push_str("0.00"); }
                } else {
                    // No posteriors - fill with zeros
                    let n_gp = n_alleles * (n_alleles + 1) / 2;
                    for i in 0..n_gp {
                        if i > 0 { self.buf.push(','); }
                        self.buf.push_str("0.00");
                    }
                }
            }

            /// Add allele probabilities (AP fields)
            fn add_ap(&mut self, n_alleles: usize, p1: Option<&crate::pipelines::imputation::AllelePosteriors>, p2: Option<&crate::pipelines::imputation::AllelePosteriors>) {
                self.field_sep();
