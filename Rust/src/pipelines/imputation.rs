...
pub struct StateProbs {
    ...
    /// Maps each genotyped marker (by sparse index) to its cluster index
    marker_to_cluster: std::sync::Arc<Vec<usize>>,
    /// Defines the start/end boundaries of each marker cluster
    clusters: std::sync::Arc<Vec<MarkerCluster>>,
}
