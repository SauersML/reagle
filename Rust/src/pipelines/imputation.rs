        ...
        info_span!("compute_dr2").in_scope(|| {
            ...
            for m in 0..n_ref_markers {
                ...
                for s in 0..n_target_samples {
                    ...
                    // For DR2 calculation, Var(d) (variance of estimated dosage) MUST be
                    // computed from HMM posteriors for ALL markers, including genotyped ones.
                    // This reflects the model's uncertainty. Var(X) (variance of true
                    // genotype) is computed from hard calls for genotyped markers.
                    ...
                    if is_genotyped {
                        // For genotyped markers, only include samples with non-missing
                        // true genotypes in the DR2 calculation.
                        if let Some(target_m) = alignment.target_marker(m) {
                            ...
                            let is_missing = a1_mapped == 255 || a2_mapped == 255;
                            if !is_missing {
                        ...
                        // For imputed markers, always include in DR2 calculation
                        if let Some(stats) = quality.get_mut(m) {
                            stats.add_sample(&probs1[..n_alleles], &probs2[..n_alleles]);
                        }
                    }
