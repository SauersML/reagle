...
impl ImputationPipeline {
    ...
    pub fn run(&mut self) -> Result<()> {
        ...
        // Run imputation PER-SAMPLE to fix data leakage bug where a marker was considered
        // "genotyped" for all samples if just one sample had data. This led to incorrect
        // sparse reference panel construction and reduced accuracy.
        //
        // Now, we determine genotyped markers and build the projected reference panel
        // for each sample individually.
        let state_probs: Vec<Arc<StateProbs>> = info_span!("run_hmm", n_samples = n_target_samples)
            .in_scope(|| {
                (0..n_target_samples)
                    .into_par_iter()
                    .map_init(
                        // Initialize workspace per thread
                        || ImpWorkspace::with_ref_size(n_states, n_ref_markers, n_ref_haps),
                        // Process each sample with its thread's workspace
                        |workspace, s| {
                            // Haplotype indices for this sample
                            ...
                            if s == 0 { // Print stats for first sample only
                                ...
                                eprintln!(
                                    "  Sample 0: HMM on {} genotyped markers, interpolating {}",
                                    n_genotyped, n_to_impute
                                );
                            }
                            
                            // 2. Build sample-specific projected reference panel
                            ...
                            // 3. Compute sample-specific clusters and HMM parameters
                            let cluster_dist = self.config.cluster as f64;
                            let clusters = compute_marker_clusters(&genotyped_markers, &gen_positions, cluster_dist);
                            ...
                            let cluster_midpoints: Vec<f64> = clusters
                                ...
                                .map(|c| {
                                    if c.end > c.start {
                                        (gen_positions[genotyped_markers[c.start]] + gen_positions[genotyped_markers[c.end - 1]]) / 2.0
                                    } else {
                            ...
                            let marker_to_cluster: Arc<Vec<usize>> = Arc::new({
                                ...
                                for (cluster_idx, cluster) in clusters.iter().enumerate() {
                                    for m in cluster.start..cluster.end { mapping[m] = cluster_idx; }
                                }
                                mapping
                            });

                            // 4. Run HMM for EACH of the two haplotypes in the sample
                            let hap_indices = [hap1_idx, hap2_idx];
                            let state_probs_pair: Vec<Arc<StateProbs>> = hap_indices.iter().map(|&hap_idx| {
                                // (Logic from previous per-haplotype loop body)
                                ...
                                let target_alleles_projected: Vec<u8> = genotyped_markers
                                    .iter().map(|&ref_m| target_alleles_dense[ref_m]).collect();
                                ...
                                let cluster_allele_match: Vec<Vec<bool>> = clusters.iter()
                                    .map(|cluster| (0..actual_n_states).map(|k| {
                                ...
                                Arc::new(StateProbs::new(
                                    genotyped_markers.clone(), actual_n_states, sparse_hap_indices, hmm_state_probs,
                                    gen_positions.clone(), marker_to_cluster.clone(),
                                ))
                            }).collect();
