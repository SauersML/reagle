        ...
        if n_alleles == 2 {
            ...
            if in_cluster {
                for (j, &hap) in haps.iter().enumerate() {
                    ...
                    if allele == 1 { p_alt += interp_prob; } else if allele == 0 { p_ref += interp_prob; }
                }
            } else {
                for (j, &hap) in haps.iter().enumerate() {
                    let allele = get_ref_allele(ref_marker, hap);
                    if allele == 1 { p_alt += probs[j]; } else if allele == 0 { p_ref += probs[j]; }
                }
                let mut p_alt_p1 = 0.0f32;
                let mut p_ref_p1 = 0.0f32;
                let haps_p1 = &self.hap_indices[start..end];
                for (j, &hap) in haps_p1.iter().enumerate() {
                    let allele = get_ref_allele(ref_marker, hap);
                    if allele == 1 { p_alt_p1 += probs_p1[j]; } else if allele == 0 { p_ref_p1 += probs_p1[j]; }
                }
