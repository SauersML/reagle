        ...
        let state_probs: Vec<Arc<StateProbs>> = info_span!("run_hmm", n_haps = n_target_haps).in_scope(|| {
            ...
            .map_init(
                ...
                |workspace, h| {
                    ...
                    // =================================================================
                    // KEY FIX: Per-haplotype "genotyped" marker detection
                    // =================================================================
                    // Instead of a single global list, we now determine which
                    // markers are genotyped FOR THIS HAPLOTYPE. This prevents
                    // data leakage and ensures the HMM only uses observed data.
                    ...
                    // --- Cluster Computation (now inside loop) ---
                    ...
                    // Build target alleles in PROJECTED space (now guaranteed no 255s)
                    ...
                    // Aggregate to cluster-level matches
                    let cluster_allele_match: Vec<Vec<bool>> = clusters
                        .iter()
                        .map(|cluster| {
                            (0..actual_n_states)
                                .map(|k| {
                                    (cluster.start..cluster.end).all(|m| sparse_allele_match[m][k])
                                })
                                .collect()
                        })
                        .collect();
                    ...
                    // Expand cluster probs to marker probs
