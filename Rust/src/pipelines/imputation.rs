...
impl ImputationPipeline {
    ...
    pub fn run(&mut self) -> Result<()> {
        ...
        // Run imputation for each target SAMPLE with per-thread workspaces
        // This is the key fix for the data leakage bug: all imputation setup
        // (genotyped marker detection, projected panel construction) is now done
        // on a per-sample basis, preventing information from other samples'
        // non-missing data from influencing the current sample's imputation.
        let state_probs: Vec<Arc<StateProbs>> = info_span!("run_hmm", n_samples = n_target_samples).in_scope(|| {
            ...
            .map_init(
                ...
                // Process each sample with its thread's workspace
                |workspace, s| {
                    let hap1_idx = HapIdx::new((s * 2) as u32);
                    let hap2_idx = HapIdx::new((s * 2 + 1) as u32);

                    // PER-SAMPLE: Determine genotyped markers for this sample only
                    let genotyped_markers_vec: Vec<usize> = (0..n_ref_markers)
                        .filter(|&ref_m| {
                            if let Some(target_m) = alignment.target_marker(ref_m) {
                                ...
                                // Check if EITHER haplotype has non-missing data
                    ...
                    // If a sample has no genotyped markers, we can't do much.
                    // Create dummy StateProbs and continue.
                    if genotyped_markers_vec.is_empty() {
                        let dummy_sp = Arc::new(StateProbs::new(
                            Arc::new(Vec::new()),
                            0,
                            Vec::new(),
                            Vec::new(),
                        ...
                        return (dummy_sp.clone(), dummy_sp);
                    }

                    // PER-SAMPLE: Compute genetic positions for projected space
                    ...
                    // PER-SAMPLE: Build coded reference panel
                    ...
                    // PER-SAMPLE: Re-initialize ImpStates with the new projected panel
                    ...
                    // PER-SAMPLE: Compute clusters and HMM parameters
                    ...
                    let marker_to_cluster: Vec<usize> = {
                        let mut mapping = vec![0usize; genotyped_markers_vec.len()];
                    ...
                    let genotyped_markers = std::sync::Arc::new(genotyped_markers_vec);

                    // Now process the two haplotypes for this sample
                    let mut sample_sps = Vec::with_capacity(2);
                    for hap_idx in [hap1_idx, hap2_idx] {
                        ...
                        sample_sps.push(Arc::new(StateProbs::new(
                            genotyped_markers.clone(),
                            actual_n_states,
                            sparse_hap_indices,
                            hmm_state_probs,
                            gen_positions.clone(),
                            marker_to_cluster.clone(),
                        )));
                    }
                    (sample_sps[0].clone(), sample_sps[1].clone())
                },
            )
            .collect::<Vec<(Arc<StateProbs>, Arc<StateProbs>)>>()
            .into_iter()
        ...
        info_span!("compute_dr2").in_scope(|| {
            ...
            for m in 0..n_ref_markers {
                ...
                for s in 0..n_target_samples {
                    ...
                    // Var(d) is always from HMM posteriors
                    let post1_est = cursor1.allele_posteriors(m, n_alleles, get_ref_allele);
                    let post2_est = cursor2.allele_posteriors(m, n_alleles, get_ref_allele);
                    for a in 0..n_alleles {
                        probs1[a] = post1_est.prob(a);
                        probs2[a] = post2_est.prob(a);
                    }

                    // Var(X) is from hard calls for genotyped, posteriors for imputed
                    let (true_probs1, true_probs2) = if is_genotyped {
                        if let Some(target_m) = alignment.target_marker(m) {
                            ...
                            let mut t_probs1 = vec![0.0; n_alleles];
                            let mut t_probs2 = vec![0.0; n_alleles];

                            if a1_mapped != 255 && (a1_mapped as usize) < n_alleles {
                                t_probs1[a1_mapped as usize] = 1.0;
                            } else {
                                for a in 0..n_alleles { t_probs1[a] = post1_est.prob(a); }
                            }

                            if a2_mapped != 255 && (a2_mapped as usize) < n_alleles {
                                t_probs2[a2_mapped as usize] = 1.0;
                            } else {
                                for a in 0..n_alleles { t_probs2[a] = post2_est.prob(a); }
                            }
                            (t_probs1, t_probs2)
                        } else {
                            (probs1.clone(), probs2.clone())
                        }
                    } else {
                        (probs1.clone(), probs2.clone())
                    };
                    ...
                    if let Some(stats) = quality.get_mut(m) {
                        stats.add_sample(
                            &probs1[..n_alleles],
                            &probs2[..n_alleles],
                            &true_probs1[..n_alleles],
                            &true_probs2[..n_alleles],
                        );
                    }
                }
            }
