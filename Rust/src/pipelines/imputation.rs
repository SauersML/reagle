        ...
        let state_probs: Vec<Arc<StateProbs>> = info_span!("run_hmm", n_haps = n_target_haps).in_scope(|| {
            ...
            .map_init(
                ...
                |workspace, h| {
                    ...
                    // =================================================================
                    // PER-HAPLOTYPE Projected Panel Construction
                    // =================================================================
                    // This is the CRITICAL fix for the imputation accuracy regression.
                    // By building the projected panel based on THIS haplotype's non-missing
                    // data, we ensure PBWT state selection is EXACT and not confused
                    // by missing data at sites that are genotyped in OTHER samples.
                    //
                    // This prevents data leakage between samples and correctly isolates
                    // the imputation problem for each haplotype.
                    
                    // 1. Determine genotyped markers FOR THIS HAPLOTYPE
                    let genotyped_markers_vec: Vec<usize> = (0..n_ref_markers)
                        .filter(|&ref_m| target_alleles_dense[ref_m] != 255)
                        .collect();
                    let n_genotyped = genotyped_markers_vec.len();
                    
                    // 2. Compute genetic positions for this haplotype's genotyped markers
                    ...
                    // 3. Build the projected reference panel
                    ...
                    // 4. Create ImpStates with this projected panel
                    ...
                    // Build target alleles in PROJECTED space
                    ...
                    // Get IBS-based states using PROJECTED PBWT
                    ...
                    // Compute marker clusters based on this haplotype's genotyped markers
                    ...
                    // If no clusters, return empty StateProbs
                    if n_clusters == 0 {
                        return Arc::new(StateProbs::new(
                            genotyped_markers,
                            0,
                            Vec::new(),
                            Vec::new(),
                    ...
                    // Cluster-level error probabilities (scaled by cluster size)
                    ...
                    let marker_to_cluster: Arc<Vec<usize>> = {
                    ...
                    // Run forward-backward HMM on CLUSTERS
