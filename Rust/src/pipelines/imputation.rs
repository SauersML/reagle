        ...
        let state_probs: Vec<Arc<StateProbs>> = info_span!("run_hmm", n_haps = n_target_haps).in_scope(|| {
            ...
            .map_init(
                ...
                |workspace, h| {
                    ...
                    // =====================================================================
                    // CRITICAL FIX: Per-sample projected space
                    // =====================================================================
                    // PROBLEM: Global `genotyped_markers` causes data leakage. A marker
                    // is "genotyped" if ANY sample has data, but THIS sample might have
                    // missing data at that site, breaking the PBWT.
                    //
                    // SOLUTION: Determine genotyped markers PER-SAMPLE and build the
                    // projected reference panel on-the-fly for this sample only.
                    // This is more expensive but guarantees correctness.
                    //
                    ...
                    // Build target alleles in PROJECTED space (NO 255s for this sample)
                    let target_alleles_projected: Vec<u8> = genotyped_markers
                        .iter()
                        .map(|&ref_m| target_alleles_dense[ref_m])
                        .collect();

                    // Per-sample cluster and HMM parameter computation
                    ...
                    let cluster_err: Vec<f32> = clusters
                        ...
                        .collect();
                    let marker_to_cluster: std::sync::Arc<Vec<usize>> = {
                    ...
                    // Create ImpStates with per-sample projected data
                    ...
                    // Get IBS-based states using per-sample PROJECTED PBWT
                            ...
                            (0..actual_n_states)
                                .map(|k| {
                                    (cluster.start..cluster.end).all(|m| {
                                        // Missing data is not an issue here since projected space is non-missing
                                        sparse_allele_match[m][k]
                                    })
                                })
                                .collect()
                        })
                    ...
                    // Run forward-backward HMM on per-sample clusters
