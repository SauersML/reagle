...
def run_benchmark(person, file_path, format):
    ...
    # We create a derivative panel where missing calls are filled (0|0) and separators forced to phased.
    # We do NOT filter missing sites (^miss) because sparse panels might become empty.
    run_cmd("bcftools +setGT ref.vcf.gz -Ou -- -t . -n 0|0 | bcftools +setGT - -Ou -- -t a -n p | bcftools view -Oz -o ref_beagle.vcf.gz", shell=True)
