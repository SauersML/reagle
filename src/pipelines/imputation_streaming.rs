...
fn compute_dosage_and_best_gt(p1: f32, p2: f32) -> (f32, (u8, u8)) {
    // We strictly use the HMM posterior probabilities for both dosage and genotype calling.
    // This ensures consistency and allows the HMM to correct errors in the input (genotype refinement).
    // p1 and p2 are probabilities of the ALT allele (index 1) for hap1 and hap2 respectively.

    let dosage = p1 + p2;

    // Calculate genotype probabilities assuming independence (HWE-like given HMM state)
    // P(0/0) = (1-p1)(1-p2)
    // P(0/1) = p1(1-p2) + (1-p1)p2
    // P(1/1) = p1*p2
    let p00 = (1.0 - p1) * (1.0 - p2);
    let p11 = p1 * p2;
    let p01 = 1.0 - p00 - p11; // Ensure sum is 1.0

    let gt = if p11 >= p01 && p11 >= p00 {
        (1, 1)
    } else if p01 >= p00 {
        (0, 1)
    } else {
