...
target_samples={} target_bytes={}",
    ...
    fn write_imputed_window_streaming<TargetSpace, RefMarkerSpace, RefPhaseSpace>(
        ...
        let get_dosage = |marker_idx: usize, sample_idx: usize| -> f32 {
            ...
            // If error correction is enabled, prioritize HMM dosages over input genotypes
            ...
            // Priority: Genotyped Alleles (GT) > Genotype Posteriors (GP) > HMM Dosage
        ...
        let get_best_gt = |marker_idx: usize, sample_idx: usize| -> (u8, u8) {
            ...
            // If error correction is enabled, prioritize HMM genotypes
            if self.config.err.is_some() {
                if let Some(result) = result_by_sample.get(sample_idx).and_then(|r| *r) {
            ...
            // Priority: Genotyped Alleles (GT) > Genotype Posteriors (GP) > HMM Genotype
