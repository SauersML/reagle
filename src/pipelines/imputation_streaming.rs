...
target_samples={} target_bytes={}",
    ...
    fn write_imputed_window_streaming(
        ...
        // Extract is_imputed status to avoid borrowing quality in closures while mutating it later
        let is_imputed_vec: Vec<bool> = quality.marker_stats.iter().map(|s| s.is_imputed).collect();
        ...
        let get_input_genotype = |marker_idx: usize, sample_idx: usize| -> Option<(u8, u8)> {
            if marker_idx < is_imputed_vec.len() && !is_imputed_vec[marker_idx] {
