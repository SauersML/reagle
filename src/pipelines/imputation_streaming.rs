...
fn calibrated_emission_error(input_probs: &TargetAlleleProbs, base_error_rate: f32) -> f32 {
    ...
    // Allow sharpening below base when typed evidence is strong, but avoid
    // collapsing all the way to a near-deterministic floor.
...
#[inline]
fn adaptive_untyped_prior_mix(p_mismatch: f32, phase_confidence_unavailable: bool) -> f32 {
    ...
    // Whole-window sparsity already feeds the HMM through the marker layout and
    // retain-based smoothing. Reusing it here double-counts uncertainty on
    // sparse arrays and flattens posteriors long after local anchor support
    // becomes strong, so keep this as a genuinely tiny floor.
...
fn score_window_batch_exact_packed<TargetSpace, RefSpace>(
    ...
    for m in 0..n_markers {
        ...
        // Direct updates avoid the large buffered sparse-accumulation pass and
        // preserve the same grouped exact-match scoring.
    ...
    >(
        ...
        let build_input_probs_pair = |hap1: HapIdx,
         ...
         -> (
            ...
            let min_untyped_prior_mix1 =
                adaptive_untyped_prior_mix(self.params.p_mismatch, !phase_conf_valid);
            let min_untyped_prior_mix2 =
                adaptive_untyped_prior_mix(self.params.p_mismatch, !phase_conf_valid);
...
mod tests {
    ...
    #[test]
    fn test_typed_marker_resolution_maps_target_biallelic_to_multiallelic_ref() {
        let chrom = ChromIdx::new(0);
        let mut target_markers = Markers::<crate::data::AnyMarkerSpace>::new();
        target_markers.add_chrom("chr1");
        target_markers.push(Marker::new(
        ...
        ref_markers.push(Marker::new(
            ...
            Some("r0".into()),
            Allele::Base(Nucleotide::C),
            vec![Allele::Base(Nucleotide::A), Allele::Base(Nucleotide::G)],
        ));
        let alignment = MarkerAlignment {
            ref_to_target: vec![None],
            target_to_ref: vec![None],
        ...
        let resolved =
            build_ref_typed_marker_resolutions(target_gt.markers(), &ref_markers, &alignment);
        let r = resolved[0].expect("expected multiallelic positional fallback resolution");
        assert_eq!(r.target_idx, 0);
        assert_eq!(r.map_kind, TypedMarkerMapKind::PositionalMatch(1, 2));
        assert_eq!(map_target_allele_to_ref(&alignment, r, 0), Some(1));
        assert_eq!(map_target_allele_to_ref(&alignment, r, 1), Some(2));

        let mut out = Vec::new();
        assert!(map_target_probs_to_ref(
            &alignment,
            r,
            &[0.7, 0.3],
            3,
