...
fn compute_dosage_and_best_gt(
    ...
    p1: f32,
    p2: f32,
    _target_win: &GenotypeMatrix<Phased>,
    _alignment: &MarkerAlignment,
) -> (f32, (u8, u8)) {
    // Always use the HMM posterior probabilities for dosage.
    // This allows the HMM to correct errors in the input genotypes (if any),
    // and ensures consistent behavior between genotyped and imputed markers.
    // The HMM already incorporates the input genotype likelihoods via emission probabilities,
    // so p1/p2 reflect the optimal combination of prior (LD) and likelihood (data).
    let dosage = p1 + p2;

    // Determine best genotype call for each haplotype independently based on posterior
    // probability threshold of 0.5. This preserves phasing information better than
    // thresholding the sum.
    let g1 = if p1 >= 0.5 { 1 } else { 0 };
    let g2 = if p2 >= 0.5 { 1 } else { 0 };
