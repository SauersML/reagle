...
fn compute_dosage_and_best_gt(p1: f32, p2: f32) -> (f32, (u8, u8)) {
    // Strictly use HMM posteriors for dosage and genotype.
    // The HMM forward-backward algorithm already incorporates the input genotype
    // via emission probabilities. Using the input genotype again here would
    // double-count the evidence and prevent error correction.
    let dosage = p1 + p2;
