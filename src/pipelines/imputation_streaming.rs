    ...
    while batch_start < n_target_haps {
        ...
        match ref_data {
            ReferenceData::InMemory {
                ...
                for (local_idx, (ref_window, ref_columns)) in
                    windows.iter().zip(packed_columns.iter()).enumerate()
                {
                    let current_window_idx = window_idx + local_idx;
                    let n_ref_markers = ref_window.markers.len();
                    if n_ref_markers == 0 {
                        continue;
                    }
                    ...
                    ) = if let Some(cache_entries) = target_cache.as_ref() {
                        if let Some(entry) = cache_entries.get(local_idx).and_then(|e| e.as_ref()) {
                            (Cow::Borrowed(&entry.alignment), Cow::Borrowed(&entry.phased_target))
                        } else {
                            continue;
                        }
                    } else {
