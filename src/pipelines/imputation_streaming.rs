...
target_samples={} target_bytes={}",
    ...
    fn write_imputed_window_streaming(
        ...
        let get_dosage = |marker_idx: usize, sample_idx: usize| -> f32 {
            let local_m = marker_idx.saturating_sub(output_start);
            // Prioritize HMM-calculated dosages (result.dosages) for the DS field to enable error correction.
            // Fall back to input genotypes only if HMM output is unavailable for this marker.
