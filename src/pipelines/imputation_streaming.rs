...
impl crate::pipelines::ImputationPipeline {
    ...
    fn write_imputed_window_streaming<TargetSpace, RefMarkerSpace, RefPhaseSpace>(
        ...
        let get_dosage = |marker_idx: usize, sample_idx: usize| -> f32 {
            ...
            // Priority:
            // 1. Explicit Genotype Posteriors (from PL)
            // 2. Hard Genotype (if error correction disabled/default) -> strict preservation
            // 3. HMM Dosage (imputed)
            // 4. Hard Genotype fallback (if enabled but HMM failed)

            // If we have PLs, use them (get_genotype_posteriors handles this)
            ...
            // If we have a hard genotype and NO explicit error rate configuration,
            // we strictly preserve the hard genotype dosage.
            // This ensures genotyped markers are not "re-imputed" away from truth.
