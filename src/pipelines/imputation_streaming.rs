...
target_samples={} target_bytes={}",
    ...
    fn write_imputed_window_streaming(
        ...
        let get_dosage = |marker_idx: usize, sample_idx: usize| -> f32 {
            ...
            // Prioritize imputed dosage if error correction is enabled (config.err is set)
            // Otherwise (hard truth), prefer genotyped alleles if available.
            let use_imputed = self.config.err.is_some();
            ...
            let dosage = if let Some(gp) = get_genotype_posteriors(marker_idx, sample_idx) {
                ...
                // If error correction is on, prefer HMM dosage
                if let Some(result) = result_by_sample.get(sample_idx).and_then(|r| *r) {
                    result.dosages.get(local_m).copied().unwrap_or(0.0)
                } else if let Some((a1, a2)) = get_genotyped_alleles(marker_idx, sample_idx) {
                    (if a1 > 0 { 1.0 } else { 0.0 }) + (if a2 > 0 { 1.0 } else { 0.0 })
                } else {
                    0.0
                }
            } else {
                // Hard truth mode: prefer genotyped alleles
