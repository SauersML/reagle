...
target_samples={} target_bytes={}",
    ...
    fn write_imputed_window_streaming(
        ...
        let get_dosage = |marker_idx: usize, sample_idx: usize| -> f32 {
            let local_m = marker_idx.saturating_sub(output_start);
            // Prioritize HMM-calculated dosage if available, even for genotyped markers.
            // This ensures DS reflects the posterior probability (allowing for error correction/refinement)
            // consistent with the "Estimated Dosage" semantics.
