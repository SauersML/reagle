...
impl ClusterStateProbs {
    ...
    pub fn biallelic_alt_prob_from_masks_simd(
        ...
        while idx + 8 <= haps.len() {
            ...
            let mut alt_arr = [0.0f32; 8];
            let mut ref_arr = [0.0f32; 8];
            for lane in 0..8 {
                let hap = haps[idx + lane] as usize;
                if hap >= alt_mask.len() {
                    continue;
                }
                if missing_mask.get(hap).copied().unwrap_or(0) != 0 {
                    continue;
                }
            ...
            let alt_vec = f32x8::from(alt_arr);
            let ref_vec = f32x8::from(ref_arr);
            p_alt += (p_vec * alt_vec).reduce_add();
            p_ref += (p_vec * ref_vec).reduce_add();
            idx += 8;
        }
        for j in idx..haps.len() {
            let hap = haps[j] as usize;
            if hap >= alt_mask.len() {
                continue;
            }
            if missing_mask.get(hap).copied().unwrap_or(0) != 0 {
        ...
        if !in_cluster {
            let missing_mass = self.missing_mass(haps.len(), noise_a, noise_b);
            let alt_freq = alt_freq.clamp(0.0, 1.0);
            p_alt += missing_mass * alt_freq;
            p_ref += missing_mass * (1.0 - alt_freq);
        }
