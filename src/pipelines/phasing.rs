     ...
     -> (
        ...
        while samples_done < lr_samples && !early_stop {
            for (c_idx, chain) in chains.iter_mut().enumerate() {
                ...
                // Stability check using first chain only for simplicity
                if c_idx == 0 {
                    ...
                    if samples_done >= min_samples {
                        ...
                        if stable_steps >= stable_required {
                            ...
                            if remaining > 0 {
                                // Fast forward counts for ALL chains
                                for c in 0..chains.len() {
                                    // Use last_obs/bits of chain 0 as proxy?
                                    // Ideally we should use per-chain bits but we only tracked prev_bits for chain 0
                                    // Given stability, chain 0 has converged. Assume others have too?
                                    // Or just stop accumulating.
                                    // Correct way: Don't fast forward other chains using Chain 0's state.
                                    // Just extrapolate using Chain 0's current state is biased if they differ.
                                    // Let's just break. The counts we have are sufficient (stable).
                                }
                            }
                            early_stop = true;
                            break;
