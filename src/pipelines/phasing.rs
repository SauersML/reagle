                    ...
                    THREAD_WORKSPACE.with(|ws| {
                        ...
                        let (swap_bits, swap_lr, swap_probs, swap_probs_conf, new_paths) =
                            if use_dynamic_mcmc {
                                ...
                                if let Some(phase_ibs) = phase_ibs.as_ref() {
                                    ...
                                    if let Some((paths, perfect)) = best_global {
                                        if prior_local.is_none() || perfect {
                                            prior_local = Some(paths);
                                            if perfect {
                                                force_full_phase = true;
                                            }
                                            if s == 0 && n_hi_freq <= 600 {
                                                let p1 = prior_local
                                                    .as_ref()
                                                ...
                                                let p2 = prior_local
                                                    ...
                                                    .unwrap_or(0);
                                                eprintln!(
                                                    "[dynamic init] sample=0 best_global_pair=({}, {}) perfect={}",
                                                    p1, p2, perfect
                                                );
                                            }
                                        }
                        ...
                        let orientation = if use_dynamic_mcmc {
                            let mut desired_hap1: Vec<(HiFreqMarkerIdx, AbsoluteHap1Allele)> =
                                Vec::with_capacity(n_hi_freq);
                            if let Some(paths) = new_paths.as_ref() {
                                if force_full_phase {
                                    for idx in 0..n_hi_freq {
                                        let a1 = seq1[idx];
                                        let a2 = seq2[idx];
                                        if a1 == 255 || a2 == 255 || a1 == a2 {
                                            continue;
...
fn sample_dynamic_mcmc(
    ...
    if !has_anchor && n_markers <= 2000 && n_states <= 512 {
        let mut informative_markers = Vec::new();
        for m in 0..n_markers {
            let a1 = seq1[m];
            let a2 = seq2[m];
            if a1 != 255 && a2 != 255 && a1 != a2 {
        ...
        if informative_markers.len() >= 8 {
            ...
            for i in 0..n_haps {
                for j in 0..i {
                    ...
                    for &m in &informative_markers {
                        let a1 = seq1[m];
                        let a2 = seq2[m];
                        let r1 = phase_ibs.allele(m, i);
                        let r2 = phase_ibs.allele(m, j);
                        if r1 == 255 || r2 == 255 {
                            perfect = false;
                            break;
                        }
            ...
            if let Some((h1_idx, h2_idx)) = best_pair {
                ...
                let path1 = vec![h1_idx as u32; n_markers];
                let path2 = vec![h2_idx as u32; n_markers];
                return (
                    swap_bits,
                    swap_lr,
                    swap_probs,
    ...
    if !has_anchor {
        if let Some(paths) = initial_paths {
            if paths.path1.len() == n_markers && paths.path2.len() == n_markers {
                ...
                let match_rate = if match_total > 0 {
                    match_count as f32 / match_total as f32
                } else {
                    0.0
                };
                if match_rate >= 0.98 {
    ...
    // Seed alleles and reference paths from initial paths if available.
    // This ensures MCMC starts in a high-probability region and keeps
    // latent states aligned with the heuristic's best constant pair.
    if let Some(paths) = initial_paths {
        if paths.path1.len() == n_markers && paths.path2.len() == n_markers {
            ...
            if all_valid {
                ...
                for m in 0..n_markers {
                    ...
                    let h2_idx = paths.path2[m] as usize;
                    if h1_idx < phase_ibs.n_haps() && h2_idx < phase_ibs.n_haps() {
                        path1_ref[m] = paths.path1[m];
                        path2_ref[m] = paths.path2[m];
                    }
                }
            }
            ...
            for m in 0..n_markers {
                ...
                let h1_idx = path1_ref[m] as usize;
                let h2_idx = path2_ref[m] as usize;
    ...
    let mut collect_dynamic_neighbors =
        |path_ref: &[u32], query_hap: &[u8], sample_idx: u32, out: &mut Vec<u32>| {
    ...
    for step in 0..n_mcmc_steps {
        ...
        collect_dynamic_neighbors(&path1_ref, &h1_alleles, sample_idx, &mut neighbors);
        ...
        collect_dynamic_neighbors(&path2_ref, &h2_alleles, sample_idx, &mut neighbors);
...
fn find_best_constant_pair_phase_ibs(
    n_markers: usize,
    seq1: &[u8],
    seq2: &[u8],
    conf: &[f32],
    ...
    if n_haps <= 2 || n_markers == 0 {
        return None;
    }
    let candidates: Vec<u32> = (0..n_haps)
        .map(|h| h as u32)
        .filter(|&h| h != hap1_idx && h != hap2_idx)
        .collect();
    if candidates.len() < 2 {
        return None;
    }
    ...
    let ops = (n_candidates as u128) * (n_candidates as u128) * (n_markers as u128);
    if ops > 100_000_000 {
        return None;
    }

    let mut scores = vec![0.0f32; n_candidates * n_candidates];
    let mut alleles = vec![255u8; n_candidates];
    let mut informative = 0usize;
    for m in 0..n_markers {
        let a1 = seq1[m];
        let a2 = seq2[m];
        ...
        let conf_m = conf.get(m).copied().unwrap_or(1.0).clamp(0.0, 1.0);
        let is_het = a1 != a2 && a1 != 255 && a2 != 255;
        for (idx, &hap) in candidates.iter().enumerate() {
            alleles[idx] = phase_ibs.allele(m, hap);
        }
        for i in 0..n_candidates {
            ...
            for j in 0..i {
                ...
                scores[i * n_candidates + j] += prob.max(1e-30).ln();
            }
        }
    }
    if informative == 0 {
    ...
    let hap1 = candidates[best_pair.0];
    let hap2 = candidates[best_pair.1];
    let path1 = vec![hap1; n_markers];
    let path2 = vec![hap2; n_markers];
    let mut perfect = true;
    for m in 0..n_markers {
        let a1 = seq1[m];
        let a2 = seq2[m];
        ...
        let r1 = phase_ibs.allele(m, hap1);
        let r2 = phase_ibs.allele(m, hap2);
        if a1 == a2 {
            let obs = if a1 != 255 { a1 } else { a2 };
            if obs != 255 && (r1 != obs || r2 != obs) {
                perfect = false;
                break;
            }
        } else {
            let matches = (r1 == a1 && r2 == a2) || (r1 == a2 && r2 == a1);
            if !matches {
                perfect = false;
                break;
            }
    ...
    Some((MosaicPaths { path1, path2 }, perfect))
}
...
fn sample_swap_bits_mosaic<RefSpace>(
    ...
    if has_anchor {
        if let Some(paths) = heuristic_paths.as_mut() {
            ...
            for m in 0..n_markers {
                ...
                let conf_m = conf[m].clamp(0.0, 1.0);
                if a1 != 255 {
                    score_direct += emit_prob(r1, a1, conf_m, p_no_err, p_err).max(1e-30).ln();
                    score_flip += emit_prob(r2, a1, conf_m, p_no_err, p_err).max(1e-30).ln();
                    evidence += 1;
                }
                if a2 != 255 {
                    score_direct += emit_prob(r2, a2, conf_m, p_no_err, p_err).max(1e-30).ln();
                    score_flip += emit_prob(r1, a2, conf_m, p_no_err, p_err).max(1e-30).ln();
                    evidence += 1;
                }
            }
    ...
    let heuristic_paths_fallback = heuristic_paths.clone();
    ...
    // or if we fall back to it. It's relatively cheap compared to the full MCMC.
    // We do it once here.
    let mut combined_checkpoints =
        FwdCheckpoints::from_buffer(block_starts.clone(), n_states_usize, combined_data);

    // Only populate combined checkpoints if we actually need them (i.e., we have a None candidate)
    if candidate_inits.iter().any(|c| c.is_none()) {
        ...
        build_fwd_checkpoints(
            ...
            &mut ref_provider,
            (!workspace.ref_alleles_flat.is_empty())
                .then_some(&workspace.ref_alleles_flat[..ref_flat_len]),
            pl_provider.as_ref(),
    ...
    // Early-stop settings (phase-level stability)
    let freeze_min_samples: usize = 8;
    let freeze_max_uncertainty: f32 = 0.01;
    // Sprint tournament settings
    let sprint_burnin: usize = 3;
    let sprint_lr_samples: usize = 4;
    let sprint_delta_per_marker: f64 = 0.005;
    let sprint_min_delta: f64 = 5.0;
     ...
     -> (
        ...
        let alloc_like = |tmpl: &MosaicBuffers| -> MosaicBuffers {
            MosaicBuffers {
                ...
                hap1_use_combined: vec![true; tmpl.hap1_use_combined.len()],
                hap1_hard_match: vec![false; tmpl.hap1_hard_match.len()],
                hap2_allele: vec![255u8; tmpl.hap2_allele.len()],
                hap2_partner_allele: vec![255u8; tmpl.hap2_partner_allele.len()],
                hap2_use_combined: vec![true; tmpl.hap2_use_combined.len()],
                hap2_hard_match: vec![false; tmpl.hap2_hard_match.len()],
                path1: vec![0u32; tmpl.path1.len()],
                path2: vec![0u32; tmpl.path2.len()],
                fwd_block: vec![0.0f32; tmpl.fwd_block.len()],
            }
        };

        let mut chains: Vec<MosaicChain<RefSpace>> = Vec::with_capacity(PAR_CHAINS);
        let mut buffers_vec: Vec<MosaicBuffers> = Vec::with_capacity(PAR_CHAINS);
        buffers_vec.push(buffers);
        for _ in 1..PAR_CHAINS {
            let tmpl = buffers_vec.first().unwrap();
            buffers_vec.push(alloc_like(tmpl));
        }
        let mut buffers_iter = buffers_vec.into_iter();
        ...
        let mut chain_last_bits = vec![vec![0u8; het_positions.len()]; PAR_CHAINS];
        let mut chain_last_obs = vec![vec![0u8; het_positions.len()]; PAR_CHAINS];

        let mut samples_done = 0usize;
        let mut stopped_early = false;
        while samples_done < lr_samples_limit {
            for (c_idx, chain) in chains.iter_mut().enumerate() {
                if samples_done >= lr_samples_limit {
                    break;
                }
                chain.step();
                ...
                if !anchor_indices.is_empty() {
                    ...
                    for &m in &anchor_indices {
                        ...
                        let conf_m = conf[m].clamp(0.0, 1.0);
                        if a1 != 255 && a2 != 255 {
                            direct += (emit_prob(r1, a1, conf_m, p_no_err, p_err)
                                * emit_prob(r2, a2, conf_m, p_no_err, p_err))
                            .max(1e-30)
                            .ln();
                            flipped += (emit_prob(r1, a2, conf_m, p_no_err, p_err)
                                * emit_prob(r2, a1, conf_m, p_no_err, p_err))
                            .max(1e-30)
                            .ln();
                            evidence += 1;
                ...
                let mut last_bits: Vec<u8> = vec![0u8; het_positions.len()];
                let mut last_obs: Vec<u8> = vec![0u8; het_positions.len()];
                for (i, &m) in het_positions.iter().enumerate() {
                    let a1 = seq1[m];
                    let a2 = seq2[m];
                    if a1 == 255 || a2 == 255 || a1 == a2 {
                    ...
                    if let Some(orient) = orient {
                        ...
                        if c_idx < chain_swap_counts.len() {
                            ...
                            agg_swap_counts[i] += orient as f32;
                            agg_obs_counts[i] += 1.0;
                            chain_last_bits[c_idx][i] = orient;
                            chain_last_obs[c_idx][i] = 1;
                        }
                    }
                }
                samples_done += 1;

                if samples_done >= freeze_min_samples {
                    let mut max_uncertainty = 0.0f32;
                    for i in 0..het_positions.len() {
                        let p = (agg_swap_counts[i] + 0.5) / (agg_obs_counts[i] + 1.0);
                        let u = 1.0 - (2.0 * p - 1.0).abs();
                        if u > max_uncertainty {
                            max_uncertainty = u;
                        }
                    }
                    if max_uncertainty < freeze_max_uncertainty {
                        stopped_early = true;
                        break;
                    }
                }
        ...
        if stopped_early && samples_done < lr_samples_limit {
            let remaining = lr_samples_limit - samples_done;
            for r in 0..remaining {
                let c_idx = (samples_done + r) % chains.len();
                for i in 0..het_positions.len() {
                    let orient = chain_last_bits[c_idx][i] as f32;
                    let obs = chain_last_obs[c_idx][i] as f32;
                    if obs > 0.0 {
                        chain_swap_counts[c_idx][i] += orient;
                        chain_obs_counts[c_idx][i] += 1.0;
                        agg_swap_counts[i] += orient;
                        agg_obs_counts[i] += 1.0;
                    }
                }
            }
        ...
        if let Some(chain) = chains.get(best_idx) {
            best_paths = MosaicPaths {
        ...
        let returned = chains.into_iter().nth(0).expect("chain 0").into_buffers();
        (
            agg_swap_counts,
            agg_obs_counts,
            best_swap_counts,
    ...
    let mut best_score = f64::NEG_INFINITY;
    let mut agg_swap_counts = vec![0.0; het_positions.len()];
    let mut agg_obs_counts = vec![0.0; het_positions.len()];
    let mut best_swap_counts = vec![0.0; het_positions.len()];
    let mut best_obs_counts = vec![0.0; het_positions.len()];
    let mut new_paths = MosaicPaths {
        path1: Vec::new(),
        path2: Vec::new(),
    };

    // Sprint tournament: short burn-in for all candidates, cull losers
    let mut candidate_keep = vec![true; candidate_inits.len()];
    if candidate_inits.len() > 1 {
        let mut sprint_scores = vec![f64::NEG_INFINITY; candidate_inits.len()];
        for (i, init) in candidate_inits.iter().enumerate() {
            ...
            if !has_anchor {
                let flipped_paths = if let Some(p) = init {
                    if p.path1.len() == n_markers && p.path2.len() == n_markers {
                        Some(MosaicPaths {
                            path1: p.path2.clone(),
                            path2: p.path1.clone(),
                        })
                ...
                if let Some(paths_f_ref) = flipped_paths.as_ref() {
                    ...
                    buffers = buf_f;
                    if score_f > best_local {
                        best_local = score_f;
                    }
                }
            }

            sprint_scores[i] = best_local;
        }

        if let Some((best_idx, best_score)) = sprint_scores
            .iter()
            .enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal))
        {
            let delta = (n_markers as f64 * sprint_delta_per_marker).max(sprint_min_delta);
            for (i, s) in sprint_scores.iter().enumerate() {
                if i == best_idx {
                    continue;
                }
                if *s < best_score - delta {
    ...
    // Run competitive chains (full)
    for (i, init) in candidate_inits.iter().enumerate() {
        if !candidate_keep.get(i).copied().unwrap_or(true) {
            continue;
        }
        let chain_seed = seed.wrapping_add(0xC0FFEE_BAAD_F00Du64.wrapping_mul((i + 1) as u64));
        ...
        for (i, &m) in het_positions.iter().enumerate() {
            if alignment_flips[m] {
                if let (Some(s), Some(o)) = (agg_sc.get_mut(i), agg_oc.get(i)) {
                    *s = *o - *s;
                }
                if let (Some(s), Some(o)) = (best_sc.get_mut(i), best_oc.get(i)) {
                    *s = *o - *s;
                }
            }
        }
        ...
        if score >= best_score {
            best_score = score;
            agg_swap_counts = agg_sc.clone();
            agg_obs_counts = agg_oc.clone();
            best_swap_counts = best_sc.clone();
            best_obs_counts = best_oc.clone();
        ...
        if !has_anchor {
            ...
            } else if paths.path1.len() == n_markers && paths.path2.len() == n_markers {
                Some(MosaicPaths {
            ...
            if flipped_paths.is_some() {
                ...
                for (i, &m) in het_positions.iter().enumerate() {
                    if alignment_flips_f[m] {
                        if let (Some(s), Some(o)) = (agg_sc_f.get_mut(i), agg_oc_f.get(i)) {
                            *s = *o - *s;
                        }
                        if let (Some(s), Some(o)) = (best_sc_f.get_mut(i), best_oc_f.get(i)) {
                            *s = *o - *s;
                        }
                    }
                }
                ...
                if score_f > best_score {
                    best_score = score_f;
                    agg_swap_counts = agg_sc_f;
                    agg_obs_counts = agg_oc_f;
                    best_swap_counts = best_sc_f;
                    best_obs_counts = best_oc_f;
                    new_paths = paths_f;
                }
            }
        }
...

    {
        ...
        for m in 1..new_paths.path1.len() {
            if new_paths.path1[m] != new_paths.path1[m - 1] {
                switches1 += 1;
            }
            if new_paths.path2[m] != new_paths.path2[m - 1] {
                switches2 += 1;
            }
        }
        let mut best_obs_mean = 0.0f32;
        let mut best_obs_nonzero = 0usize;
        for &v in &best_obs_counts {
            best_obs_mean += v;
            if v > 0.0 {
                best_obs_nonzero += 1;
            }
        }
        if !best_obs_counts.is_empty() {
            best_obs_mean /= best_obs_counts.len() as f32;
        }
        let suspicious_paths =
            !has_anchor && het_positions.len() >= 8 && (switches1 + switches2) > n_markers / 6;
        if suspicious_paths {
            eprintln!(
                "[mosaic static path anomaly] seed={} markers={} hets={} path_switches=({},{}) best_score={:.3} best_obs_nonzero={}/{} best_obs_mean={:.2}",
                seed,
                n_markers,
                het_positions.len(),
                switches1,
        ...
        let fallback_candidate = if heuristic_paths_fallback.is_some() {
        ...
        if let Some(fallback_paths) = fallback_candidate.as_ref() {
            let paths_have_len =
                fallback_paths.path1.len() == n_markers && fallback_paths.path2.len() == n_markers;
            let paths_have_states = paths_have_len
                && fallback_paths
                    .path1
                    .iter()
                    .all(|&p| (p as usize) < n_states_usize)
                && fallback_paths
                    .path2
                    .iter()
                    .all(|&p| (p as usize) < n_states_usize);

            if paths_have_states {
                let mut fallback_switches1 = 0usize;
                let mut fallback_switches2 = 0usize;
                for m in 1..fallback_paths.path1.len() {
                    if fallback_paths.path1[m] != fallback_paths.path1[m - 1] {
                        fallback_switches1 += 1;
                    }
                    if fallback_paths.path2[m] != fallback_paths.path2[m - 1] {
                        fallback_switches2 += 1;
                    }
                }

                let mut fallback_ll = 0.0f64;
                let mut match_count = 0usize;
                let mut match_total = 0usize;
                if !shared_ref.is_empty() {
                    for m in 0..n_markers {
                        ...
                        let p1 = fallback_paths.path1[m] as usize;
                        let p2 = fallback_paths.path2[m] as usize;
                        if p1 >= n_states_usize || p2 >= n_states_usize {
                            continue;
                        }
                        let offset = m * n_states_usize;
                        let ref_row = &shared_ref[offset..offset + n_states_usize];
                        let ref1 = ref_row[p1];
                        let ref2 = ref_row[p2];
                        let conf_m = conf[m];
                        match_total += 1;
                    ...
                    for m in 0..n_markers {
                        ...
                        fallback_provider.fill_ref_alleles(m, &mut ref_alleles);
                ...
                let fallback_prior = calculate_log_prior(
                    &fallback_paths.path1,
                    &fallback_paths.path2,
                    p_recomb,
                    n_states_usize,
                );
                let fallback_score = fallback_ll + fallback_prior;
                let score_tolerance = n_markers as f64 * 0.01;
                let improves_switching =
                    (fallback_switches1 + fallback_switches2) < (switches1 + switches2);
                let acceptable_score = fallback_score + score_tolerance >= best_score;
                let match_rate = if match_total > 0 {
                    match_count as f32 / match_total as f32
                } else {
                    0.0
                };
                let strong_match = match_rate >= 0.98;
                if !has_anchor && improves_switching && (strong_match || acceptable_score) {
                    new_paths = fallback_paths.clone();
                    eprintln!(
                        "[mosaic static path fallback] seed={} switches=({},{}) -> ({},{}) score={:.3} fallback_score={:.3} match_rate={:.3}",
                        seed,
                        switches1,
                        switches2,
                        fallback_switches1,
        ...
        && new_paths.path2.len() == n_markers
    {
        ...
        if has_anchor {
            ...
            for m in 0..n_markers {
                ...
                let p1 = new_paths.path1.get(m).copied().unwrap_or(0) as usize;
                let p2 = new_paths.path2.get(m).copied().unwrap_or(0) as usize;
                if p1 >= n_states_usize || p2 >= n_states_usize {
                    continue;
                }
                ...
                let ref2 = ref_row[p2];
                let conf_m = conf[m].clamp(0.0, 1.0);
                let keep = emit_prob(ref1, s1, conf_m, p_no_err, p_err)
                    * emit_prob(ref2, s2, conf_m, p_no_err, p_err);
                let swap = emit_prob(ref1, s2, conf_m, p_no_err, p_err)
                    * emit_prob(ref2, s1, conf_m, p_no_err, p_err);
                score_direct += keep.max(1e-30).ln();
                score_flip += swap.max(1e-30).ln();
                evidence += 1;
            }
            if evidence > 0 && score_flip > score_direct {
                flip_to_anchor = true;
    ...
    if !het_positions.is_empty() {
        let mut pre_bits = vec![0u8; het_positions.len()];
        let mut pre_probs = vec![0.5f32; het_positions.len()];
        let mut pre_mid = 0usize;
        for i in 0..het_positions.len() {
            let p = swap_probs.get(i).copied().unwrap_or(0.5).clamp(0.0, 1.0);
            pre_probs[i] = p;
            pre_bits[i] = if p > 0.5 { 1 } else { 0 };
            if (p - 0.5).abs() < 0.05 {
                pre_mid += 1;
            }
        }
        let mut pre_transitions = 0usize;
        for i in 1..pre_bits.len() {
            if pre_bits[i] != pre_bits[i - 1] {
                pre_transitions += 1;
            }
        }

        let suspicious_emission = !has_anchor
            && pre_probs.len() >= 8
            && pre_mid == 0
            && pre_transitions * 3 > pre_probs.len();
        if suspicious_emission {
            for i in 0..swap_probs.len() {
                swap_probs[i] = 0.5;
                swap_lr[i] = 1.0;
            }
        ...
        for i in 0..het_positions.len() {
            ...
            if i == 0 {
                ...
                let from1_to0 = dp1[i - 1] + sw;
                if from0_to0 >= from1_to0 {
                    dp0[i] = from0_to0 + emit0_log[i];
                    prev_state[i] = 0;
                } else {
                    dp0[i] = from1_to0 + emit0_log[i];
                    prev_state[i] = 1;
                }
                ...
                let from1_to1 = dp1[i - 1] + stay;
                if from0_to1 >= from1_to1 {
                    dp1[i] = from0_to1 + emit1_log[i];
                } else {
                    dp1[i] = from1_to1 + emit1_log[i];
                    prev_state[i] |= 2;
                }
            }
        }
        ...
        {
            let mut post_transitions = 0usize;
            for i in 1..swap_bits.len() {
                if swap_bits[i] != swap_bits[i - 1] {
                    post_transitions += 1;
                }
            }
            if !has_anchor && pre_probs.len() >= 20 {
                let preview_len = pre_probs.len().min(12);
                eprintln!(
                    "[mosaic static summary] seed={} anomaly={} pre_mid={} pre_transitions={} post_transitions={} label_switch={:.3e} pre_probs={:?} pre_bits={:?} post_bits={:?}",
                    seed,
                    suspicious_emission,
                    pre_mid,
                    pre_transitions,
    ...
    if swap_bits.len() >= 8 {
        ...
        if post_transitions * 3 > swap_bits.len() {
            let mean_swap = if swap_probs.is_empty() {
                0.5
            } else {
                swap_probs.iter().sum::<f32>() / swap_probs.len() as f32
            };
            if (mean_swap - 0.5).abs() < 0.1 {
                let forced = if mean_swap > 0.5 { 1 } else { 0 };
                swap_bits.fill(forced);
                swap_lr.fill(1.0);
                eprintln!(
                    "[mosaic swap stabilization] seed={} hets={} transitions={} mean_swap={:.3} forced={}",
                    seed,
                    swap_bits.len(),
                    post_transitions,
                    mean_swap,
