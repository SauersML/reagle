...
fn sample_dynamic_mcmc(
    ...
    // This ensures MCMC starts in the high-probability region found by the heuristic,
    // rather than drifting from a random start.
        ...
        if paths.path1.len() == n_markers && paths.path2.len() == n_markers {
            for m in 0..n_markers {
                ...
                if a1 == 255 || a2 == 255 || a1 == a2 {
                    continue; // Skip homs/missing
                }

                // Look up alleles from the hint paths
                // Indices in paths are global haplotype indices
                let h1_idx = paths.path1[m] as usize;
                let h2_idx = paths.path2[m] as usize;

                // Only use if indices are valid in the PBWT
                if h1_idx < phase_ibs.n_haps() && h2_idx < phase_ibs.n_haps() {
                    ...
                    // Check if hints are consistent with genotype
                    ...
                    // If both match (impossible for hets unless ref1==ref2) or neither match, keep random
                }
            }
        }
    }
