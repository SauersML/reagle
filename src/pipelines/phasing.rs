                    ...
                    |(s, (mask, het_lr_out, het_phase_out, paths_out, cohort_stats_out_one))| {
                        ...
                        *het_phase_out = het_positions
                            ...
                            .map(|(idx, &m)| {
                                ...
                                let p_orient = calibrate_orientation_confidence(
                                    p_orient,
                                    swap_lr.get(idx).copied().unwrap_or(1.0),
                    ...
                    THREAD_WORKSPACE.with(|ws| {
                        ...
                        for (map_idx, &idx) in het_positions.iter().enumerate() {
                            ...
                            let confidence = {
                                ...
                                let p_orient = calibrate_orientation_confidence(
                                    p_orient,
                                    *swap_lr.get(map_idx).unwrap_or(&1.0),
                                    self.params.lr_threshold,
                                );
                                PhaseConfidence(p_orient)
                            };
        ...
        for (s, decision) in phase_decisions.into_iter().enumerate() {
            ...
            } in &decision.het_updates
            {
                ...
                sp.set_phase_confidence(
                    m,
                    calibrate_orientation_confidence(*p_orient, 1.0, self.params.lr_threshold),
                );
            }
...
fn decode_orientation_hmm(
    ...
    for i in 0..n {
        ...
        let tempered_p1 = {
            const ORIENTATION_POSTERIOR_GAMMA: f32 = 0.35;
            let logit = (p1 / (1.0 - p1)).ln();
            1.0 / (1.0 + (-ORIENTATION_POSTERIOR_GAMMA * logit).exp())
        };
        posterior_p_swap[i] = tempered_p1;

        let p_chosen = if states[i] == 1 {
            tempered_p1
        } else {
            1.0 - tempered_p1
        };
...
#[inline]
fn calibrate_orientation_confidence(p_orient: f32, lr: f32, lr_threshold: f32) -> f32 {
    let centered = p_orient.clamp(0.0, 1.0) - 0.5;
    let scale = if lr_threshold > 1.0 {
        (lr.max(1.0).ln() / lr_threshold.ln()).clamp(0.0, 1.0)
    } else {
        1.0
    };
    (0.5 + centered * scale).clamp(0.0, 1.0)
}
