...
impl<'a> BeagleHmm<'a> {
    ...
    pub fn conditioned_forward_backward(
        ...
        let fill_conf_emissions =
            |m: usize, partner: u8, emissions: &mut [f32], ref_alleles: &[u8]| {
                let conf = conf_at(m);
        ...
        for m in 0..n_markers {
            ...
            if m == 0 {
                ...
                fwd_sum = HmmUpdater::fwd_update_emissions(
                    curr_row, fwd_sum, p_recomb_m, &emissions, n_states,
                );
            }
        ...
        for m in (0..n_markers - 1).rev() {
            ...
            // Normalize bwd to prevent underflow
            let mut sum = 0.0f32;
            let current_bwd = &mut bwd[curr_row..curr_row + n_states];
            for val in current_bwd.iter() {
                sum += *val;
            }
            if sum > 0.0 {
                let scale = 1.0 / sum;
                for val in current_bwd.iter_mut() {
                    *val *= scale;
                }
            }
        }
    ...
    pub fn conditioned_forward_backward_with_lookup(
        ...
        let fill_conf_emissions = |m: usize, partner: u8, emissions: &mut [f32]| {
            ...
            if let Some(req) = required_allele(m, partner) {
                ...
                let p_err = p_err_base * conf + 0.5 * (1.0 - conf);
                for k in 0..n_states {
                    emissions[k] = if lookup.allele(m, k) == req {
                        p_no_err
                    } else {
                        p_err
                    };
...
mod tests {
    ...
    fn test_fwd_update_preserves_probability_mass() {
        ...
        for n_states in [4, 8, 16, 32] {
            ...
            let emissions: Vec<f32> = mismatches.iter().map(|&m| emit_probs[m as usize]).collect();
            let new_sum =
                HmmUpdater::fwd_update_emissions(&mut fwd, initial_sum, 0.05, &emissions, n_states);
    ...
    fn test_simd_vectorized_matches_scalar() {
        ...
        for n_states in [8, 11, 16, 17, 24, 25] {
            ...
            let emissions: Vec<f32> = mismatches.iter().map(|&m| emit_probs[m as usize]).collect();
            let new_sum =
                HmmUpdater::fwd_update_emissions(&mut fwd, initial_sum, 0.02, &emissions, n_states);
    ...
    fn test_extreme_recombination_rates() {
        ...
        let emissions: Vec<f32> = mismatches.iter().map(|&m| emit_probs[m as usize]).collect();
        let new_sum = HmmUpdater::fwd_update_emissions(
            &mut fwd_no_recomb,
            initial_sum,
            0.0,
            &emissions,
    ...
    fn test_numerical_stability_small_values() {
        ...
        let emissions: Vec<f32> = mismatches.iter().map(|&m| emit_probs[m as usize]).collect();
        let new_sum =
            HmmUpdater::fwd_update_emissions(&mut fwd, initial_sum, 0.01, &emissions, n_states);
    ...
    fn test_bwd_update_constant_normalization() {
        ...
        let constant_term = 2.0; // sum(bwd * emissions)
