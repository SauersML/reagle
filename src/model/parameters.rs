...
mod tests {
    ...
    fn test_li_stephens_p_mismatch() {
        // Small N: probability decreases as N increases
        let p100 = ModelParams::li_stephens_p_mismatch(100);
        let p200 = ModelParams::li_stephens_p_mismatch(200);
        assert!(p100 > p200, "Probability should decrease with N (before clamp)");
        assert!(p200 > ModelParams::MIN_MISMATCH_PROB, "Should be above clamp for N=200");

        // Large N: probability clamped to MIN_MISMATCH_PROB
        let p10k = ModelParams::li_stephens_p_mismatch(10000);
        let p20k = ModelParams::li_stephens_p_mismatch(20000);
        assert_eq!(p10k, ModelParams::MIN_MISMATCH_PROB, "Should be clamped for large N");
        assert_eq!(p20k, ModelParams::MIN_MISMATCH_PROB, "Should be clamped for large N");
        assert_eq!(p10k, p20k, "Should remain constant after clamp");
