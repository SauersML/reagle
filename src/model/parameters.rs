...
impl ModelParams {
    ...
    pub fn for_phasing(n_haps: usize, ne: f32, err: Option<f32>) -> Self {
        ...
        // Reduced scaling from 0.04 to 0.01 to improve stability in dense maps
        let recomb_intensity = (0.01 * ne / n_haps as f32).min(Self::MAX_RECOMB_INTENSITY);

        // Enforce a minimum p_mismatch to prevent overfitting to noisy input
        // when users specify unrealistically low error rates.
        let p_mismatch = err
            .unwrap_or_else(|| Self::li_stephens_p_mismatch(n_haps))
            .max(0.04);
...
mod tests {
    ...
    fn test_recomb_intensity_formula() {
        ...
        // Should be 0.01 * 100_000 / 1000 = 1.0 (Reduced from 0.04)
        let expected = 0.01 * 100_000.0 / 1000.0;
