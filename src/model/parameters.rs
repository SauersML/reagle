...
impl ModelParams {
    ...
    /// (e.g. Ne=200K, 200 haps → 40.0). Excessive switching allows the model
    /// to overfit noise by stitching too many short segments.
    /// However, empirical tests show Reagle requires higher intensity (200+)
    /// for small panels to match accuracy, so we cap at 300.0.
    pub const MAX_RECOMB_INTENSITY: f32 = 300.0;
