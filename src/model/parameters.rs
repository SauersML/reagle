...
mod tests {
    ...
    fn test_recomb_intensity_formula() {
        ...
        // Should be 0.04 * 100_000 / 1000 = 4.0 (below max)
        let expected = 0.04 * 100_000.0 / 1000.0;
        assert!((params.recomb_intensity - expected as f32).abs() < 0.01);

        // Check clamping
        let params_high = ModelParams::for_phasing(1000, 1_000_000.0, None);
        // Formula: 40.0 -> Clamped to 5.0
        assert!((params_high.recomb_intensity - ModelParams::MAX_RECOMB_INTENSITY).abs() < 0.001);
    }
