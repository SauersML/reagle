...
impl ModelParams {
    ...
    /// Maximum recombination intensity to prevent excessive switching in small panels
    ...
    pub fn for_imputation(
        ...
        let raw_intensity = 0.04 * ne / n_ref_haps as f32;
        let recomb_intensity = raw_intensity.min(Self::MAX_RECOMB_INTENSITY);
...
mod tests {
    ...
    fn test_recomb_intensity_formula() {
        ...
        // Should be 0.04 * 1_000_000 / 1000 = 40.0, but clamped to 5.0
        let calculated = 0.04 * 1_000_000.0 / 1000.0;
        assert!(calculated > ModelParams::MAX_RECOMB_INTENSITY as f64);
        assert!((params.recomb_intensity - ModelParams::MAX_RECOMB_INTENSITY).abs() < 0.0001);

        // Test below clamp
        let params2 = ModelParams::for_phasing(1000, 1000.0, None);
        let expected2 = 0.04 * 1000.0 / 1000.0; // 0.04
        assert!((params2.recomb_intensity - expected2 as f32).abs() < 0.0001);
    }
