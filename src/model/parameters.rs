...
impl ModelParams {
    ...
    /// (e.g. Ne=200K, 200 haps → 4000.0). A low cap makes the HMM
    /// too "sticky", causing AF collapse and poor dosage calibration.
    pub const MAX_RECOMB_INTENSITY: f32 = 20000.0;
    ...
    pub fn for_phasing(n_haps: usize, ne: f32, err: Option<f32>) -> Self {
        // Formula from Li & Stephens (2003): rho = 4 * Ne / n
        // Previous 0.04 factor was likely a unit conversion error (cM vs Morgan).
