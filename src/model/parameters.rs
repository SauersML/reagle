...
impl ModelParams {
    ...
    /// Minimum mismatch probability to avoid numerical issues and trap in perfect LD
    ...
    pub fn for_imputation(
        ...
        // Note: We default to MIN_MISMATCH_PROB (1e-9) instead of Li-Stephens approximation
        // to avoid the "Perfect LD Trap" where the penalty for switching to a rare haplotype (p_recomb)
        // is higher than the penalty for mismatching the common haplotype (p_mismatch).
        // In strong LD, p_recomb is small, so p_mismatch must be even smaller to force a switch.
