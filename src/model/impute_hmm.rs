...
fn smooth_allele_posteriors_subset(
    ...
    // Pseudo-count mass tracks information decay, with a gain factor to
    // explicitly tune the strength of the prior. A gain of 2.0 maintains
    // structure without excessive washout.
    //
    // Note: prior_mass should NOT be scaled by effective_alleles (N), as
    // that would cause 'more matches = less confidence' paradox.
    const SMOOTHING_GAIN: f32 = 2.0;
    let prior_mass = (SMOOTHING_GAIN * (1.0 - retain) / retain).max(0.0);
    if prior_mass <= 0.0 {
        return;
    }
