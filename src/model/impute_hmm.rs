...
fn apply_marker_prior_smoothing(
    ...
    // Use linear sparsity penalty: the fraction of the panel we ignored.
    // This represents the probability that the true haplotype is in the unselected set,
    // in which case we should fall back to the panel prior (marginal).
    let sparsity_boost = (1.0 - active_ratio).max(0.0);
    ...
    let adaptive_panel_mix = (base_mix * (1.0 + 0.5 * sparsity_boost)).clamp(0.0, 0.95);
