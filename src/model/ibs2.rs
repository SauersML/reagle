...
impl Ibs2 {
    ...
    fn is_ibs2_at(gt: &GenotypeMatrix, marker: usize, s1: SampleIdx, s2: SampleIdx) -> bool {
        ...
        let informative = (a1 != crate::data::storage::AlleleCode::MISSING.raw()
            && b1 != crate::data::storage::AlleleCode::MISSING.raw())
            || (a2 != crate::data::storage::AlleleCode::MISSING.raw()
                && b2 != crate::data::storage::AlleleCode::MISSING.raw())
        ...
        for m in 0..n_markers {
            ...
            if marker_maf >= Self::MIN_MINOR_FREQ && marker_maf <= 1.0 - Self::MIN_MINOR_FREQ {
                ...
                for h in 0..gt.n_haplotypes() {
                    if gt.allele(m_idx, HapIdx::new(h as u32))
        ...
        for step in 0..n_steps {
            ...
            for s in 0..n_samples {
                ...
                for &m in &step_markers {
                    let m_idx = MarkerIdx::new(m as u32);
                    let sample = SampleIdx::new(s as u32);
                    if gt.allele(m_idx, sample.hap1())
                        == crate::data::storage::AlleleCode::MISSING.raw()
                        || gt.allele(m_idx, sample.hap2())
                            == crate::data::storage::AlleleCode::MISSING.raw()
                    {
                        miss_cnt += 1;
                    }
